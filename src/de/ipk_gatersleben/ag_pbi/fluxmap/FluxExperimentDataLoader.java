...
public class FluxExperimentDataLoader extends TemplateLoader {
	...
	private HashMap<String, Double> getSubstanceWeights(TableData tabledata) {
		HashMap<String, Double> substanceWeights = new HashMap<String, Double>();
	...
	private ExperimentInterface getExperimentDataFromFile(final File file, TableData tabledata, BackgroundTaskStatusProviderSupportingExternalCall status) {
		...
		for (int col = 5; col <= tabledata.getMaximumCol(); col++) {
			String substance = tabledata.getUnicodeStringCellData(col, 20);
			if (substance == null)
				continue;
			
			substance = trimSpaceChars(substance);
			...
			ExperimentInterface e = processReaction(file, tabledata, substance, conditions, samples,
									reactionname, col);
	...
	private ExperimentInterface processReaction(final File file, TableData tabledata,
						String reactionString, HashMap<Integer, ConditionInterface> conditions,
						HashMap<Integer, SampleInterface> samples,
						String reactionname,
						int col) {
		ExperimentInterface experiment = new Experiment();
		
		FluxReaction reaction = new FluxReaction(reactionString);
		...
		for (int row = 23; row <= tabledata.getMaximumRow(); row++) {
			...
			for (FluxReactant fr : reaction.getAllReactants()) {
				...
				substance.setInfo(reaction.getInfo(reactionname));
				...
				if (od != null) {
					NumericMeasurementInterface md = getMeasurementData(tabledata, sample, col, row, od, quality, fr);
					sample.add(md);
				}
	...
	private SampleInterface getSampleData(TableData tabledata, int row) {
		...
		// time
		Object timeObj = tabledata.getCellData(col("B"), row, null);
		...
		// time unit
		if (tabledata.getUnicodeStringCellData(col("C"), row) != null)
			sample.setTimeUnit(tabledata.getUnicodeStringCellData(col("C"), row));
		else
	...
	private NumericMeasurementInterface getMeasurementData(TableData tabledata,
		...
		Double weight = substanceWeight.get(fr.getName().toLowerCase());
		if (weight != null)
	...
	public static String trimSpaceChars(String str) {
		...
		int index;
		index = start;
		while (str.charAt(index) == 160 | str.charAt(index++) == 32)
			start++;
		index = end;
		while (str.charAt(index) == 160 | str.charAt(index--) == 32)
			end--;
